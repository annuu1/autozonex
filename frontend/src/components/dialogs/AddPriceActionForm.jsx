...
const AddPriceActionForm = ({ onSubmit, onCancel, priceAction }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 flex flex-wrap gap-4">
      ...
      <textarea
        value={formData.source_timeframes?.map(f => `${f.timeframe}-${f.zone_type}-${f.zone_price_range || ""}-${f.reason_price_reached_here || ""}`).join("\n")}
        onChange={(e) =>
          handleArrayFieldChange("source_timeframes",
            e.target.value.split("\n").filter(Boolean).map(line => {
              const [timeframe, zone_type, zone_price_range, reason_price_reached_here] = line.split("-");
              return { timeframe, zone_type, zone_price_range, reason_price_reached_here };
            })
          )
        }
      ...
      <textarea
        value={formData.key_levels?.map(k => `${k.level_type}-${k.price}`).join("\n")}
        onChange={(e) =>
          handleArrayFieldChange("key_levels",
            e.target.value.split("\n").filter(Boolean).map(line => {
      ...
      <input
        ...
        onChange={(e) => handleArrayFieldChange("trade_setups", e.target.value.split(",").map(v => v.trim()))}
      ...
      <input
        ...
        onChange={(e) => handleArrayFieldChange("positives", e.target.value.split(",").map(v => v.trim()))}
      ...
      <input
        ...
        onChange={(e) => handleArrayFieldChange("negatives", e.target.value.split(",").map(v => v.trim()))}
